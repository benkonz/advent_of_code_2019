    ...
    for wire1_segment in wire1_path {
        ...
        for wire2_segment in &wire2_path {
            ...
            if let Some(intersection) = get_intersection(
                ...
                if intersection != (1, 1) {
                    let wire1_point_distance =
                        get_distance(wire1_segment, intersection, wire1_current_start);
                    let wire2_point_distance =
                        get_distance(*wire2_segment, intersection, wire2_current_start);

                    let total_distance = wire1_distance
                        + wire2_distance
                        + wire1_point_distance
                        + wire2_point_distance;
                    intersections.push(total_distance);
                }
            }
            wire2_current_start = wire2_current_end;
            wire2_distance += (*wire2_segment).1;
        }
        wire1_current_start = wire1_current_end;
        wire1_distance += wire1_segment.1;
    }
...
fn get_distance(segment: (char, i64), start_point: (i64, i64), end_point: (i64, i64)) -> i64 {
    match segment {
        (dir, _) if dir == 'R' || dir == 'L' => i64::abs(start_point.0 - end_point.0),
        (dir, _) if dir == 'U' || dir == 'D' => i64::abs(start_point.1 - end_point.1),
